...
BEGIN
  IF TG_OP = 'UPDATE' AND
     ...
     OLD.face_descriptor IS NOT DISTINCT FROM NEW.face_descriptor AND
  ...
  -- Skip face-similarity check when enrolling a descriptor for the first time
  -- on an existing persona (was NULL, now being populated). This allows
  -- backfilling descriptors even when stylized depictions look similar.
  IF NEW.face_descriptor IS NOT NULL
     AND jsonb_array_length(NEW.face_descriptor) = 128
     AND NOT (TG_OP = 'UPDATE' AND OLD.face_descriptor IS NULL) THEN
    SELECT p.id, p.name,
      sqrt(
        (SELECT sum(power(
